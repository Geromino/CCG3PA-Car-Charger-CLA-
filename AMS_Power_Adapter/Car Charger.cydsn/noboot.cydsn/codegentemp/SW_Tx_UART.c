...
#include "SW_Tx_UART_PVT.h"
    ...
    uint32 SW_Tx_UART_pinNumber = SW_Tx_UART_PIN_NUMBER;
    uint32 SW_Tx_UART_pinPortNumber = SW_Tx_UART_PIN_PORT_NUMBER;
    #if(CY_PSOC3)
        uint32 pdata SW_Tx_UART_pinDrAdress = SW_Tx_UART_PIN_DR;
    #else
        uint32 SW_Tx_UART_pinDrAdress = SW_Tx_UART_PIN_DR;
    #endif /* (CY_PSOC3) */
#else
    uint32 SW_Tx_UART_pinNumber;
    uint32 SW_Tx_UART_pinPortNumber;
    #if(CY_PSOC3)
        uint32 pdata SW_Tx_UART_pinDrAdress;
    #else
        uint32 SW_Tx_UART_pinDrAdress;
    #endif /* (CY_PSOC3) */
...
#if(SW_Tx_UART_PIN_STATIC_MODE == 1u)
    /*******************************************************************************
    * Function Name: SW_Tx_UART_Start
    ********************************************************************************
    *
    * Summary:
...
#else
    /*******************************************************************************
    * Function Name: SW_Tx_UART_StartEx
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void SW_Tx_UART_StartEx(uint8 port, uint8 pin) 
    {
        ...
        if ((pin <= SW_Tx_UART_MAX_PIN_NUMBER) && (port <= SW_Tx_UART_MAX_PORT_NUMBER))
        {
            #if (!CY_PSOC4)
                portConfigAddr = SW_Tx_UART_PORT_CNF_BASE;
                portConfigAddr += ((uint32)port * (SW_Tx_UART_MAX_PIN_NUMBER + 1u)) + pin;
                CyPins_SetPinDriveMode(portConfigAddr, CY_PINS_DM_STRONG);
                CyPins_SetPin(portConfigAddr);
                SW_Tx_UART_pinDrAdress = portConfigAddr;
            #else
                portConfigAddr = SW_Tx_UART_PORT_CNF_BASE + (SW_Tx_UART_PORT_CNF_SIZE * port) +
                                                                                SW_Tx_UART_PORT_CNF_MODE_OFFSET;
                CY_SYS_PINS_SET_DRIVE_MODE(portConfigAddr, pin, CY_SYS_PINS_DM_STRONG);
                portDataRegAddr = SW_Tx_UART_PORT_CNF_BASE + (SW_Tx_UART_PORT_CNF_SIZE * port) +
                                                                                SW_Tx_UART_PORT_CNF_DR_OFFSET;
                CY_SYS_PINS_SET_PIN(portDataRegAddr, pin);
                SW_Tx_UART_pinDrAdress = portDataRegAddr;
            #endif /* (!CY_PSOC4) */
            SW_Tx_UART_pinNumber = pin;
            SW_Tx_UART_pinPortNumber = port;
        }
    }
#endif /* (SW_Tx_UART_PIN_STATIC_MODE == 1u) */
...
/*******************************************************************************
* Function Name: SW_Tx_UART_PutString
********************************************************************************
*
* Summary:
...
void SW_Tx_UART_PutString(const char8 string[]) 
{
    ...
    {
        SW_Tx_UART_PutChar((uint8)current);
...
/*******************************************************************************
* Function Name: SW_Tx_UART_PutArray
********************************************************************************
*
* Summary:
...
void SW_Tx_UART_PutArray(const uint8 array[], uint32 byteCount) 
{
    ...
    {
        SW_Tx_UART_PutChar(array[arrayIndex]);
    }
}


/*******************************************************************************
* Function Name: SW_Tx_UART_PutHexByte
********************************************************************************
*
* Summary:
...
void SW_Tx_UART_PutHexByte(uint8 txHexByte) 
{
    ...
    SW_Tx_UART_PutChar((uint8) SW_Tx_UART_hex[txHexByte >> SW_Tx_UART_BYTE_UPPER_NIBBLE_SHIFT]);
    SW_Tx_UART_PutChar((uint8) SW_Tx_UART_hex[txHexByte & SW_Tx_UART_BYTE_LOWER_NIBBLE_MASK]);
}


/*******************************************************************************
* Function Name: SW_Tx_UART_PutHexInt
********************************************************************************
*
* Summary:
...
{
    SW_Tx_UART_PutHexByte((uint8)(txHexInt >> SW_Tx_UART_U16_UPPER_BYTE_SHIFT));
    SW_Tx_UART_PutHexByte((uint8)(txHexInt & SW_Tx_UART_U16_LOWER_BYTE_MASK));
}


/*******************************************************************************
* Function Name: SW_Tx_UART_PutCRLF
********************************************************************************
*
* Summary:
...
{
    SW_Tx_UART_PutChar(0x0Du);
    SW_Tx_UART_PutChar(0x0Au);
}
